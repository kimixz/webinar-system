...
router.post("/", async (req, res) => {
    ...
    try {
        ...
        const tokenData = await jwtVerify(token, process.env.AUTH_SECRET)
...
router.put("/:id", async (req, res) => {
    try {
        ...
        if (req.body.isRequestAccept) {
            ...
            const sendingText = `درخواست برگزاری وبینار شما با موضوع ${req.body.webinarTitle} ${req.body.isAccepted ? 'قبول' : 'رد'} شد`;

            var transporter = nodemailer.createTransport({
                service: 'gmail',
                auth: {
                    user: 'webinarsystemmanage@gmail.com',
                    pass: 'webinar1234'
                }
            });
            ...
            var mailOptions = {
                from: 'webinarsystemmanage@gmail.com',
                to: req.body.lecturerEmail,
                subject: 'نتیجه درخواست برگزاری وبینار',
                text: sendingText
            };
